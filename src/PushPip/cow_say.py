...
# without usage argument
class Parser:
    def __init__(self):
        self.parser = argparse.ArgumentParser(description="cowsay console wrapper used argparse packet",
                                         prog="cowsay wrapper",
        ...
        self.parser.add_argument("message", action='store', nargs='+', help='the message to be displayed')
        info_group = self.parser.add_argument_group("informative options", "only debugging output in console without running cowsay")
        info_group.add_argument("-l", "--list_cows", action='store_true', help='available cows can be found by calling list_cows')
        self.parser.add_argument('--usage', action='store_true', help='execute print_usage method')
        self.parser.add_argument('--version', action='version', help='get current version', version='%(prog)s v1.0.0') # alias for prog parameter
        self.parser.add_argument('--verbose', action='store_true', help='display cow name and where is located (absolute path to file)')

        text_group = self.parser.add_argument_group("text display setting", "customize output")
        ...
        cow_groups = self.parser.add_argument_group('cow customization', description='options that control how the entity will look')
        ...
        cow_opt_group = self.parser.add_argument_group('cow options', 'use yet prepared tongue and eyes from the box')
        ...
        cow_opt_group2 = self.parser.add_argument_group('edit appearance of cow', 'both parameter can be used at the same time')
    ...
    def parse(self, cl_args=None):
        if cl_args is None: cl_args = sys.argv[1:]
